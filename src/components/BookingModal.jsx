...
//   const modalBase = 'bg-deep-3 border border-gold/15 rounded-brand-xl p-10 w-full max-w-lg transition-all duration-300'
...
//           </div>
//           <button onClick={onClose} className="text-brand-dim hover:text-brand-text transition-colors duration-200 text-xl leading-none ml-4">✕</button>
//         </div>
...
//             <button onClick={onClose} className="bg-transparent border border-white/15 text-brand-muted px-6 py-2.5 rounded-full text-sm transition-all duration-200 hover:border-white/30 hover:text-brand-text">
//               Cancel
//             </button>
//             <button onClick={handleSubmit} className="flex-1 bg-gold text-deep font-semibold py-2.5 rounded-full text-sm transition-all duration-200 hover:bg-gold-light">
//               Send Enquiry →
//             </button>
//           </div>
...
function BookingModal({ show, onClose, onSubmit, preselectedService }) {
  ...
  const overlayBase = 'fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/75 backdrop-blur-sm transition-all duration-300'
  const overlayVisible = show ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
  const modalBase = 'bg-deep-3 border border-gold/15 rounded-brand-xl p-10 w-full max-w-lg transition-all duration-300 max-h-[90vh] overflow-y-auto'
  const modalVisible = show ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95'
